...
class PointEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, num_neighbors=16):
        ...
        self.norm2 = nn.BatchNorm2d(in_chans)
        self.conv2 = nn.Conv2d(in_chans, embed_dim, 1, bias=False)
        self.norm3 = nn.BatchNorm2d(embed_dim)
        self.conv3 = nn.Conv2d(embed_dim, embed_dim, 1, bias=False)
    ...
    def forward(self, x):
        ...
        # Embedding
        neigh_emb = self.conv2(self.norm2(neigh_emb))
        neigh_emb = self.conv3(self.norm3(neigh_emb)).max(-1)[0]
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        w1 = self.x_embedder.conv1.weight.data
        w2 = self.x_embedder.conv2.weight.data
        w3 = self.x_embedder.conv3.weight.data
        w4 = self.x_embedder.final.weight.data
...
DiT3D_models_CrossAttnPointPlus = {
    'DiT-XS/4':  DiT_XS_4,
}
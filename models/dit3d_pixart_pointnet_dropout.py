...
class ConditionEmbedder(nn.Module):
    ...
    """
    def __init__(self, hidden_size, uncond_prob, frequency_embedding_size=256, num_conditions=3, num_cyclic_conditions=1):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(frequency_embedding_size, hidden_size, bias=True),
            nn.SiLU(),
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, t, context=None):
        ...
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa)).reshape(B, N, C)
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y, c, force_dropout=False):
        ...
        # t = t + box_dims
        # t0 = self.t_block(t)
        # box_pos = self.c_embedder(box_pos, self.training, force_drop_ids=force_dropout)
